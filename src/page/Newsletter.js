  ...
  render() {
    ...
    return(
      <Container style={{backgroundColor: '#fdfdfd'}}>
          <Navbar left={left} title="Đăng ký nhận tin tức" />
            <View style={{flex: 1, justifyContent: 'center', alignItems: 'center', paddingLeft: 50, paddingRight: 50}}>
              <Item>
                  <Icon active name='ios-mail-outline' />
                  <Input placeholder='Địa chỉ email' onChangeText={(text) => this.setState({email: text})}/>
              </Item>
              <View style={{alignItems: 'center'}}>
                <Button onPress={() => this.subscribe()} style={{backgroundColor: Colors.navbarBackgroundColor, marginTop: 20}}>
                  <Text style={{color: '#fdfdfd'}}> Theo dõi </Text>
                </Button>
              </View>
            </View>
      </Container>
